...
public class ParticleGeneratorCommand
{
	...
	private static int help(CommandContext<CommandSourceStack> ctx)
	{
		...
		for (String str : text)
		{
			ctx.getSource().sendSuccess(new TextComponent(str), false);
		}
	...
	{
		ctx.getSource().sendSuccess(new TextComponent(ParticleGenerator.getFullName()), false);
		ctx.getSource().sendSuccess(new TextComponent("Author: mt1006"), false);
		return 1;
	}
}
